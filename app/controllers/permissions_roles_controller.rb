class PermissionsRolesController < ApplicationController
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to permissions_roles_path, status: :see_other, notice: "Permissions role was successfully destroyed." }
  ...
  private
    ...
    def set_permissions_role
      @permissions_role = PermissionsRole.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def permissions_role_params
      params.expect(permissions_role: [ :permission_id, :role_code ])
    end
end
